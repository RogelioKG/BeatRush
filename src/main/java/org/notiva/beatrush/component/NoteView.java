...
public class NoteView extends StackPane { // 可以是 StackPane 或其他你喜歡的 Node 容器
    ...
    public static final double NOTE_WIDTH = 130; // 音符的大
    public static final double NOTE_HIGH = 18; // 音符的大
     ...
     * @param targetY 音符最終到達的 Y 座標 (判定線)
    ...
    public NoteView(Note noteData, double startY, double targetY, double dropDuration, double laneX) {
        ...
        dropAnimation.setFromY(0); // 從當前位置開始 (相對於 setLayoutY)
        dropAnimation.setToY(targetY - startY); // 移動到目標 Y 座標的相對距離
